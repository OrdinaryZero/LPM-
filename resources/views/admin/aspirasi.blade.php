...
<head>
    ...
    <title>Kelola Aspirasi - Admin LPM</title>
...
<body class="bg-gray-100 h-screen flex overflow-hidden">
    ...
    <aside class="w-64 bg-[#1f2328] text-white flex flex-col hidden md:flex h-full shadow-2xl relative z-20">
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-2">
            ...
            <div class="pt-4">
                ...
                <a href="{{ route('admin.aspirasi.index') }}" class="flex items-center gap-3 px-4 py-3 bg-[#d4a017] text-white rounded-xl font-bold transition-colors shadow-[0_4px_15px_rgba(212,160,23,0.3)]">
    ...
    <main class="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header class="h-20 bg-white shadow-sm flex items-center justify-between px-8 shrink-0 z-10">
            <div>
                <h2 class="text-xl font-extrabold text-gray-800">Manajemen Aspirasi & Keluhan Warga</h2>
                <p class="text-xs font-medium text-gray-500">Tinjau dan tindak lanjuti masukan yang dikirimkan masyarakat</p>
            </div>
        ...
        <div class="flex-1 overflow-y-auto p-8">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden flex flex-col">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    <h3 class="font-extrabold text-gray-800 text-lg">Kotak Masuk Aspirasi</h3>
                </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50/50 text-xs text-gray-400 uppercase tracking-wider">
                                <th class="p-4 pl-6 font-bold border-b border-gray-100 w-1/4">Identitas Pelapor</th>
                                <th class="p-4 font-bold border-b border-gray-100 w-1/6">Kategori</th>
                                <th class="p-4 font-bold border-b border-gray-100 w-1/3">Detail Laporan</th>
                                <th class="p-4 font-bold border-b border-gray-100 text-center">Status Pengerjaan</th>
                            </tr>
                        </thead>
                        <tbody class="text-sm divide-y divide-gray-50">
                            @forelse($data_aspirasi as $item)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="p-4 pl-6">
                                    <p class="font-bold text-gray-800">{{ $item->nama }}</p>
                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $item->no_hp) }}" target="_blank" class="text-xs font-bold text-green-600 hover:underline flex items-center gap-1 mt-1">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.893-4.443 9.893-9.892 0-5.447-4.446-9.892-9.893-9.892-5.452 0-9.893 4.445-9.893 9.892 0 1.988.546 3.791 1.558 5.405l-1.125 4.103 4.268-1.115zm-1.055-10.741c-.26-.568-.535-.579-.785-.589-.204-.008-.439-.009-.684-.009-.246 0-.645.093-.982.465-.337.373-1.285 1.258-1.285 3.067 0 1.809 1.316 3.56 1.5 3.809.183.249 2.596 3.96 6.29 5.56 3.693 1.6 3.693 1.066 4.367.973.674-.093 2.174-.888 2.48-1.748.305-.86.305-1.598.214-1.748-.092-.15-.337-.242-.705-.428-.368-.186-2.174-1.074-2.511-1.198-.337-.123-.583-.186-.828.187-.246.372-.947 1.197-1.162 1.446-.214.249-.429.28-.797.093-.368-.186-1.552-.572-2.956-1.821-1.093-.974-1.83-2.176-2.046-2.548-.214-.373-.023-.574.161-.76.155-.156.368-.429.552-.643.184-.215.246-.373.368-.622.122-.249.061-.466-.03-.652-.093-.186-.828-1.996-1.134-2.73z"/></svg>
                                        Hubungi WA
                                    </a>
                                </td>
                                <td class="p-4 text-gray-600 font-bold">
                                    <span class="bg-gray-100 px-2.5 py-1 rounded-md border border-gray-200">{{ $item->kategori }}</span>
                                </td>
                                <td class="p-4">
                                    <p class="text-xs text-gray-600 bg-gray-50 p-2.5 rounded-lg border border-gray-100 leading-relaxed">
                                        {{ $item->pesan }}
                                    </p>
                                    <p class="text-[10px] text-gray-400 mt-2">Masuk: {{ $item->created_at->format('d M Y - H:i') }} WITA</p>
                                </td>
                                <td class="p-4">
                                    <form action="{{ route('admin.aspirasi.status', $item->id) }}" method="POST" class="flex justify-center">
                                        @csrf
                                        ...
                                        ">
                                            <option value="Menunggu" {{ $item->status == 'Menunggu' ? 'selected' : '' }}>Menunggu Validasi</option>
                                            <option value="Diproses" {{ $item->status == 'Diproses' ? 'selected' : '' }}>Sedang Dikerjakan</option>
                                            <option value="Selesai" {{ $item->status == 'Selesai' ? 'selected' : '' }}>Tuntas / Selesai</option>
                                        </select>
                                    </form>
                                </td>
                            </tr>
